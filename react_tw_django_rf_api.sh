...
pipenv run django-admin startproject "${PROJECT_NAME}project" . \
  || { echo "❌ django-admin startproject failed"; exit 1; }
pipenv run python3 manage.py startapp "${PROJECT_NAME}api" \
  || { echo "❌ startapp failed"; exit 1; }
touch ./db.sqlite3
...
echo ""
echo -e "Run ${BOLD}${CYAN}pipenv shell${RESET} to start your environment."
echo -e "Run ${BOLD}${CYAN}python manage.py runserver 8000${RESET} in your project root (${PROJECT_NAME}api) to run your API server."
echo ""
